...
**GMM/HMM 应当保留。** 单独看它们确实很差：任何配置下都赢不了 climatology（0.686），也赢不了 baseline（0.753），最好一档 0.825，集成权重也只有约 10%。但单独分数不是判断依据，边际贡献才是。把两者权重归零重测：校准集成的前瞻 Brier 从 0.6208 退到 0.6386，仍然赢 climatology，但**技能优势缩水了 27%**（0.065 → 0.047）。逐日配对的 Brier 差为 +0.0175，分块 bootstrap 95% CI **[+0.0090, +0.0266]，不含零**。
