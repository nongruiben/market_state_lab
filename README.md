...
### 仓位 haircut 是偏好旋钮，不是最优解

`reports/exposure_tradeoff.csv` 每次运行输出完整权衡曲线。0.45 以上的每一档都显著优于纯波动率目标化，且回撤单调改善到边界——**数据无法替你选级别，只能给你标价**：

| haircut | 年化 | 最大回撤 | 平均回撤 | CDaR | 换手 | Calmar |
|---|---|---|---|---|---|---|
| 0.00 | 7.24% | −25.74% | −4.81% | −19.38% | 0.0170 | 0.281 |
| 0.45 | 6.29% | −19.91% | −4.02% | −15.66% | 0.0150 | 0.316 |
| **0.65（默认）** | **5.86%** | **−17.74%** | **−3.72%** | **−14.31%** | 0.0142 | **0.330** |
| 0.85 | 5.42% | −16.91% | −3.46% | −13.24% | 0.0134 | 0.321 |
| 1.00 | 5.09% | −16.28% | −3.29% | −12.53% | 0.0128 | 0.313 |

默认取 0.65 的依据是 **Calmar 在此处取到内部极大值**（0.281 → 0.330 → 回落到 0.313）。这和回撤列不同——回撤单调改善到边界、无法给出级别，Calmar 则确实指向一个点。但它只是一个准则：若你只在乎回撤，一直调到 1.00 也成立。线性 haircut 已胜过平方型和硬阈值型，形状是定了的。

### 两条口径修正

**不再假设可以加杠杆。** `exposure_cap` 默认 1.0。波动率目标化在最平静的日子想要 1.36 倍，全样本有 19% 的天数会超过 100%；压到 1.0 只少 0.25 个百分点年化，回撤反而更浅。回测不应该悄悄假设一个读者拿不到的仓位。

**评估的和建议的现在是同一个东西。** 此前报告叫你按平滑后的 `decision_*` 行动，`decision_value` 却把 haircut 施加在未平滑的概率上——回测的是没人在跑的策略。现在两边都用平滑后的权重，换手率因此从 0.0302 降到 0.0170，且三个状态变体全部显著优于 vol_only（此前只有校准版显著）。

已知口径不一致（尚未处理）：报告建议你按平滑后的 `decision_*` 权重行动，但 `decision_value` 与 `exposure_tradeoff` 里的 haircut 施加在**未平滑**的 `calibrated_p_high_risk` 上。两个评估函数彼此一致，但与报告的行动建议不一致。
