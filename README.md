...
|---|---|---|---|---|---|---|
| 0.00 | 7.47% | −26.04% | −5.29% | −19.61% | 0.0302 | 0.287 |
| 0.45 | 6.48% | −21.05% | −4.59% | −16.13% | 0.0344 | 0.308 |
| **0.65（默认）** | **6.02%** | **−18.77%** | **−4.33%** | **−14.65%** | 0.0383 | **0.321** |
| 0.85 | 5.55% | −17.30% | −4.23% | −13.72% | 0.0428 | 0.321 |
| 1.00 | 5.19% | −16.20% | −4.20% | −13.03% | 0.0465 | 0.320 |

默认取 0.65 的依据是 Calmar 在此处到达拐点（0.287 → 0.321 后走平）：再往上只是继续付出收益和换手，风险调整后不再改善。线性 haircut 也已胜过平方型和硬阈值型，所以**形状是定了的，级别仍然由你定**。改数字、重跑、看那张表即可。
